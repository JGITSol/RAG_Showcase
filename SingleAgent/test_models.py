...
class SingleAgentModelTestSuite:
    ...
    def __init__(self):
        # Load available models from repo config to avoid hardcoded lists
    cfg_path = Path(__file__).resolve().parents[2] / 'config' / 'models.json'
        try:
            with open(cfg_path, 'r', encoding='utf-8') as f:
                cfg = json.load(f)
            self.available_models = [
                cfg.get('ollama', {}).get('primary_model'),
                cfg.get('ollama', {}).get('embedding_model'),
                cfg.get('ollama', {}).get('reranker_model')
            ]
        except Exception:
            self.available_models = [
                "gemma3:latest",
    ...
    async def test_model_configuration(self, primary_model: str, embedding_model: str, reranker_model: Optional[str] = None) -> Dict:
        """Test SingleAgent RAG with specific model configuration"""
    print("\n🔬 Testing configuration:")
    print("   Primary:", primary_model)
    print("   Embedding:", embedding_model)
    print("   Reranker:", reranker_model or 'None')
    ...
    print("📊 Testing", len(primary_models), "primary models with:")
    print("   Embedding:", embedding_model)
